...
internal class Program
{
    ...
    private static string _accessTokenExpiresIn;
    ...
    private static void InitializeProperties()
    {
        ...
        _appKey = "";
        _appSecret = "";
        _client = new TikTokAPIClient(_baseURI, _accessToken, _accessTokenExpiresIn, _refreshToken, _appKey, _appSecret, _apiVersion);
        _attributes = new List<ProductAttribute>();
        _command = Command.BuildCategoryBreadCrumbs;
    ...
    private static void Main(string[] args)
    {
        ...
        switch (_command)
        {
            case Command.GenerateReport:
                HandleGenerateReport();
                break;
            case Command.GenerateEnum:
                HandleGenerateEnum();
                break;
            case Command.GenerateUnorderedList:
                HandleGenerateUnorderedList();
                break;
            case Command.FindMatchingKeys:
                HandleFindMatchingKeys();
                break;
            case Command.BuildCategoryBreadCrumbs:
                HandleBuildCategoryBreadCrumbs();
                break;
            case Command.GenerateFieldDescriptions:
                HandleGenerateFieldDescriptions();
                break;
            case Command.DeleteDuplicateProducts:
                HandleDeleteDuplicateProducts();
                break;
            default:
    ...
    private static void HandleGenerateEnum()
    {
    ...
    private static void HandleGenerateUnorderedList()
    {
    ...
    private static void HandleFindMatchingKeys()
    {
    ...
    private static void HandleDeleteDuplicateProducts()
    {
        var tiktokProducts = TikTokProductImport.GetBasicProductData(_client);

        if (tiktokProducts != null && tiktokProducts.Any())
        {
            var duplicateProducts = GetDuplicateProducts(tiktokProducts);
            DeleteDuplicateProducts(duplicateProducts);
        }
        else
        {
            throw new Exception("Invalid command");
    ...
    private static List<GetTikTokProduct> GetDuplicateProducts(List<GetTikTokProduct> tiktokProducts)
    {
        ...
        var productsToKeep = duplicateGroups.Select(group => group.First()).ToList();
    ...
    private static void DeleteDuplicateProducts(List<GetTikTokProduct> duplicateProducts)
    {
        if (duplicateProducts != null && duplicateProducts.Any())
        {
            var productIds = duplicateProducts.Select(product => product.Id).ToList();
            var groupedProductIds = GroupProductIds(productIds, 20);
            ...
            foreach (var group in groupedProductIds)
            {
                ...
                _client.SendRequest<DeleteProducts>(HttpMethod.Delete, "/api/products",
                    JsonConvert.SerializeObject(jsonBody), "");
            }
        }
    }

    private static List<List<string>> GroupProductIds(List<string> productIds, int groupSize)
    {
        var groupedProductIds = new List<List<string>>();

        for (int i = 0; i < productIds.Count; i += groupSize)
        {
            groupedProductIds.Add(productIds.Skip(i).Take(groupSize).ToList());
        }

...
internal class Program
{
    ...
    private static void InitializeProperties()
    {
        ...
        _accessToken = "TTP_ZSIb3QAAAACgC9Q2hWu8S9I1n-DRzFC-hIPII_9L_Oa4pxQ0UN2kwuDFA_BbDwZqOvgUYXY81sTCuxxcW2cDhmHtFnQcC_t9Z4dMvwJWhaVhVJNbjYDXDPCVjqqxPefwaPIy4YqTyTGA-SYGXygXpgJbvd42N3VTiNuxxZjTCOsAaSA0eyp76Q";
    ...
    private static void Main(string[] args)
    {
        ...
        else if (_command == Command.GenerateEnum)
        {
            ...
            string filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads", $"{enumName}.cs");

            GenerateEnum(enumNamesInput, enumValuesInput, filePath, enumName);
        }
        else if (_command == Command.GenerateUnorderedList)
        {
            ...
            string filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads", "UnorderedList.cs");

            ConvertListToHtml(listInput, filePath);
        }
        else if (_command == Command.FindMatchingKeys)
        {
            foreach (var category in _tikTokCategories)
            {
                ...
                if (productAttributes.IsSuccess && productAttributes?.Data?.Attributes != null)
                {
                    Console.WriteLine("Category: {0}, Contains Fields: {1}, Matching Keys: {2}", category.Key,
                        AreAllKeysPresent(productAttributes.Data.Attributes, _requiredFields), GetMatchingKeys(productAttributes.Data.Attributes, _requiredFields));
                    Console.WriteLine("\n");
                }
            }
        }
    ...
    public static void ConvertListToHtml(string listInput, string filePath)
    {
    ...
    private static bool AreAllKeysPresent(List<ProductAttribute> attributes, List<string> keys)
    {
    ...
    private static List<string> _requiredFields = new List<string>
    {
        "Category",
        "Net Weight",
        "Pet Type",
    ...
    private static Dictionary<string, string> _tikTokCategories = new Dictionary<string, string>()
        {
